...
import { IUpdateEvento } from '@modules/eventos/domain/models/IUpdateEvento';
...
class EventoRepository implements IEventoRepository {
  ...
  async update(id: string, evento: IUpdateEvento): Promise<IEvento |null> {
    await this.model.updateOne({ _id: id }, evento)
